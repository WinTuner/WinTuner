  ...
  return (
    <div ref={sectionRef} className="space-y-6">
      {posts.map((post, index) => (
        ...
        >
          {post.externalUrl ? (
            <a href={post.externalUrl} target="_blank" rel="noopener noreferrer" className="absolute inset-0 z-10">
              <span className="sr-only">{t.srRead} {post.title}</span>
            </a>
          ) : (
            <Link href={`/blog/${post.slug}?lang=${language}`} className="absolute inset-0 z-10">
              <span className="sr-only">{t.srRead} {post.title}</span>
            </Link>
          )}
